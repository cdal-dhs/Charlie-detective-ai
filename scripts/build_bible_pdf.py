...
class BibleDoc(BaseDocTemplate):
    def __init__(self, filename: str, **kw) -> None:
        super().__init__(
            ...
            topMargin=2 * cm,
            bottomMargin=2 * cm,
            title="Bible Detective.be Agent",
            author="CDAL — Digital HS",
            **kw,
        )
...
def build_story() -> list:
    ...
    s.append(
        p(
            "Daniel Hurchon, détective privé belge, gère seul 4 boîtes mail (4 marques) "
    ...
    s.append(
        table_header(
            [
                ...
                ["Detectives Belgique", "info@detectives-belgique.be", "FR", "francophones BE"],
            ],
    ...
    s.append(
        code(
            "[4 boîtes IMAP via app passwords]\n"
        ...
        table_header(
            [
                ...
                [
                    ...
                    "Switch facile Kimi K2 / OpenRouter",
                ],
                [
                    "LLM principal",
                    "Kimi K2 via Ollama Pro (20€/mois)",
                    "Qualité top, coût plafonné",
                ],
                ["LLM fallback", "OpenRouter (Claude / GPT-4o)", "Spécialisation par tâche"],
                [
                    "Embeddings",
                    "intfloat/multilingual-e5-large",
                    "Gratuit, local CPU, FR/NL/EN excellent",
                ],
    ...
    s.append(
        p("Pour chaque mail classé <font face='Courier'>demande_client</font>, le générateur :")
    )
    for item in [
    ...
    s.append(
        code(
            ...
            "MAILBOX_4_NAME=detectives_belgique\n"
            "MAILBOX_4_USER=info@detectives-belgique.be\n"
            "MAILBOX_4_APP_PASSWORD=\n"
            "MAILBOX_4_BRAND=Detectives Belgique\n"
            "MAILBOX_4_DEFAULT_LANG=fr\n"
            "MAILBOX_4_IMAP_HOST=ex5.mail.ovh.net\n\n"
            "# --- LLM (LiteLLM router) ---\n"
            "OLLAMA_PRO_API_KEY=\n"
    ...
    s.append(
        p(
            "• <b>Sujet</b> : <font face='Courier'>[AGENT][${marque}] ${sujet du mail original}</font>",
            BULLET,
        )
    )
    s[-1].bulletText = ""
    ...
    s.append(
        table_header(
            [
                ...
                ["Hébergement mail (4 boîtes)", "déjà payé"],
    ...
    for item in [
        "Déposer les 4 fichiers .sqlite anonymisés dans data/",
        'Partager le schéma de chaque DB (sqlite3 ... ".schema")',
        "Remplir .env avec : 4 app passwords IMAP, OLLAMA_PRO_API_KEY, RESEND_API_KEY",
    ...
    for item in [
        ...
        "Tests sur 1 boîte d'abord, puis les 4",
    ...
    tests = [
        (
            "IMAP",
            "Injecter mail test depuis compte externe vers chacune des 4 boîtes → "
            "détection < 5 min + tag $AgentProcessed posé",
        ),
        (
            "Classification",
            "1 mail de chaque catégorie (newsletter, facture, demande client, spam) "
            "→ classification correcte dans logs",
        ),
        (
            "RAG",
            "Mail demande client similaire à un cas historique → top-5 contient le cas attendu",
        ),
        ("Génération", "Brouillon respecte langue du mail entrant + signature de la bonne marque"),
        (
            "Livraison",
            "Email reçu sur cdal@digitalhs.biz avec format complet (métadonnées + brouillon "
            "+ original + cas RAG)",
        ),
        (
            "Multilingue",
            "3 mails identiques FR/NL/EN → 3 brouillons cohérents chacun dans sa langue",
        ),
        (
            "Robustesse",
            "Kill brutal du process → systemd redémarre dans 10s, reprend sans doublon "
        ...
        (
            "Charge",
            "Injecter 100 mails simultanés → tous traités sans crash, latence moyenne < 30s/mail",
        ),
    ]
    s.append(
        table_header(
    ...
    glossary = [
        (
            "App password",
            "Mot de passe applicatif Infomaniak distinct du mot de passe principal, "
            "permet une révocation par boîte sans tout casser.",
        ),
        (
            "$AgentProcessed",
            "Flag IMAP custom posé sur les mails entrants après traitement par "
            "l'agent, pour garantir l'idempotence (un mail = un brouillon).",
        ),
        (
            "e5-large",
            "Modèle d'embeddings multilingue intfloat/multilingual-e5-large, gratuit, "
            "tourne en CPU. Convention : préfixer les passages par « passage: » et les "
            "requêtes par « query: ».",
        ),
        (
            "Few-shot RAG",
            "Technique de génération qui injecte 3-5 exemples concrets (paires Q/R "
            "retrouvées par similarité) directement dans le prompt pour faire imiter "
            "un style sans fine-tuning.",
        ),
        (
            "Kimi K2",
            "Modèle de langue de Moonshot AI, accessible via Ollama Pro (abonnement "
            "20€/mois). Performances comparables à Claude / GPT-4 sur la rédaction multilingue.",
        ),
        (
            "LiteLLM",
            "Bibliothèque Python qui expose une API unique compatible OpenAI mais route "
            "vers n'importe quel provider (Ollama, OpenRouter, Anthropic, etc.).",
        ),
        (
            "OpenRouter",
            "Marketplace de LLMs avec une seule API, utilisé ici en fallback et pour "
            "tester d'autres modèles à la demande.",
        ),
        (
            "Resend",
            "Service d'envoi d'emails transactionnels par API HTTP, free tier 3000 mails/mois.",
        ),
        (
            "sqlite-vec",
            "Extension SQLite récente qui ajoute des opérateurs vectoriels (recherche "
            "par similarité) à n'importe quelle DB SQLite. Évite d'avoir un service "
            "vectoriel séparé.",
        ),
        (
            "structlog",
            "Bibliothèque de logs structurés JSON (vs texte plat), parsable par les "
            "outils de monitoring.",
        ),
        (
            "systemd unit",
            "Fichier de configuration qui décrit un service à long terme géré par "
