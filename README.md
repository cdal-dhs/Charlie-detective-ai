...
Version source de vérité : **`pyproject.toml`** (`version = "1.5.3"`).

Le badge affiché dans le cockpit (`v1.5.3`) est lu dynamiquement depuis `pyproject.toml` via `importlib.metadata`. Ne modifier la version que dans `pyproject.toml`.
